...
def generate_grad_cam(model, img_path, class_idx):
    ...
    gradient_fn = K.function(
        [model.input], [layer_output, grads, model.layers[-1].output])
...
def uploader():
    if request.method == 'POST':
        ...
        img_path = "./static/uploader/original/"+request.files['file'].filename
        f.save(img_path)
        ...
        props = generate_bbox(img, cam, 0.3)
        ...
        return 'http://'+ip+'/static/uploader/heatmap/'+request.files['file'].filename
